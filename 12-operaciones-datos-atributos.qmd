...
En este capítulo se ejemplifican, para datos vectoriales, la creación de subconjuntos (`select()`, `slice()`, `filter()`), la agregación (`group_by()` y `summarize()`) y la unión (`left_join()` y otros *joins*) con los paquetes sf y dplyr; y, para datos raster, la creación de subconjuntos y el resumen de información con el paquete terra.
