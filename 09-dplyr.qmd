# dplyr - gramática para manipulación de datos {#sec-dplyr}
...
- [Delitos cometidos en Costa Rica en 2025](https://sitiooij.poder-judicial.go.cr/phocadownload/Estadisticas/estadsticaspoliciales2025.xls), disponibles en el [sitio Datos Abiertos del Organismo de Investigación Judicial (OIJ)](https://sitiooij.poder-judicial.go.cr/index.php/ayuda/servicios-policiales/servicios-a-organizaciones/indice-de-transparencia-del-sector-publico-costarricense/datos-abiertos).
...
### Delitos cometidos en Costa Rica en 2025
...
```{r}
#| label: carga-estadisticaspoliciales2025
#| message: false

# Carga de los datos de delitos cometidos en 2025
delitos <- read_csv(
  "https://raw.githubusercontent.com/gf0604-procesamientodatosgeograficos/2026-i/main/datos/oij/estadisticaspoliciales2025.csv"
)
```
...
```{r}
#| label: estructura-estadisticaspoliciales2025

# Estructura de los datos de delitos
glimpse(delitos)
```

Despliegue de los primeros registros de los datos en una tabla con formato:

```{r}
#| label: vista-estadisticaspoliciales2025
#| eval: true
...
La función [group_by()](https://dplyr.tidyverse.org/reference/group_by.html) agrupa las filas de un data frame con base en los valores de una o más columnas. Generalmente, esto se hace con el objetivo de realizar cálculos para cada grupo (por ejemplo, promedios, sumas o conteos por categoría).
...
# Mínimo, máximo, promedio de masa y cantidad de individuos
...
En los siguientes pasos la persona estudiante practica el uso combinado de los verbos de dplyr (`select()`, `filter()`, `arrange()`, `mutate()`, `group_by()`, `summarize()`) y del operador *pipe* (`|>`) para responder preguntas sobre el conjunto de datos `penguins` del paquete `palmerpenguins`.

### Preparación

1. Abra el proyecto `exploracion-palmerpenguins` creado en el ejercicio del @sec-tidyverse. Si no lo tiene disponible, créelo siguiendo los pasos 1 a 3 de ese capítulo.

2. En el proyecto, cree un nuevo *script* de R con la opción *File - New File - R Script* y guárdelo como `dplyr-penguins.R`. Al inicio del *script*, cargue los paquetes `tidyverse` y `palmerpenguins`.

### Pipelines de dplyr

Para cada una de las siguientes preguntas, escriba en el *script* un *pipeline* de dplyr que produzca el resultado solicitado, ejecútelo y agregue un comentario breve interpretando la salida.

3. ¿Cuántos individuos de cada sexo hay en cada especie?

4. ¿Cuál es el mínimo, máximo y promedio de masa corporal (peso) por especie y sexo?

5. ¿Cuántos individuos se observaron durante cada año?

6. ¿Cuántos individuos de cada especie se observaron durante cada año?

7. ¿Cuántos individuos de cada especie y cada sexo se observaron durante cada año?

8. ¿Cuál es el promedio de masa corporal (peso) por año?

9. ¿Cuál es el promedio de masa corporal (peso) por año para cada especie?

### Cierre

10. Guarde el *script* y ejecútelo completo (con el botón *Source* o el atajo `Ctrl + Shift + S` en Windows/Linux, `Cmd + Shift + S` en macOS). Verifique que todas las salidas se generen sin errores.
