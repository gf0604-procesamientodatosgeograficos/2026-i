...
Hasta ahora se utilizó la sintaxis básica de ggplot2 (datos, mapeos y geometrías). Para algunos tipos de gráficos —especialmente los de barras— es útil ampliar ese esquema con tres elementos adicionales: la transformación estadística (`stat`), el posicionamiento de las geometrías (`position`) y el sistema de coordenadas (`coord_*`):
...
grafico_dispersion_ggplot2 <-
  ...
  geom_point(aes(
    text = paste0(
      ...
      "\n",
      "PIB per cápita: ",
      number(GDP_PC, accuracy = 1, prefix = "$", big.mark = ","),
      "\n",
      "Esperanza de vida: ",
      number(LIFE_EXPECTANCY, accuracy = 0.1, suffix = " años")
    )
  )) +
  geom_smooth(method = "lm") +
  scale_x_continuous(labels = label_comma(), limits = c(0, NA)) +
  scale_y_continuous(labels = label_comma(), limits = c(50, 90)) +
  ggtitle("PIB per cápita vs esperanza de vida al nacer") +
  xlab("PIB per cápita (USD)") +
  ylab("Esperanza de vida (años)") +
  labs(caption = "Fuentes: Natural Earth y Banco Mundial") +
...
histograma_ggplot2 <- 
  ...
  geom_histogram(
    aes(
      text = paste0(
        "PIB per cápita (valor medio del rango): ", number(after_stat(x), accuracy = 1, prefix = "$", big.mark = ","), "\n",
...
El siguiente gráfico de pastel muestra la distribución de la población según las regiones de la Organización de Naciones Unidas (ONU).
...
En este caso no se presenta una versión interactiva del gráfico, ya que la función `ggplotly()` no admite el sistema de coordenadas polares (`coord_polar()`) que utiliza ggplot2 para construir el gráfico de pastel.
