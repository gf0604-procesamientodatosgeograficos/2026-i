...
Además de los paquetes del ecosistema espacial y de graficación ya empleados en capítulos anteriores (`tidyverse`, `sf`, `plotly`, `DT`, `leaflet`), este capítulo introduce tres paquetes nuevos:
...
Como ejemplo integral, se transformó el tablero de control de riqueza de félidos del @sec-tableros-control en una aplicación interactiva. La aplicación incorpora una **barra lateral** con dos filtros —especie y área de conservación—, una fila superior con tres **cajas de valor reactivas** y tres salidas —un mapa, un gráfico y una tabla—; todas estas salidas se actualizan en respuesta a esos filtros.

- [Aplicación publicada en shinyapps.io](https://mfvargas.shinyapps.io/riqueza-felidos/)
- [Repositorio de la aplicación en GitHub](https://github.com/gf0604-procesamientodatosgeograficos/2026-i-aplicacion-riqueza-felidos)
- [Código fuente de la aplicación](https://github.com/gf0604-procesamientodatosgeograficos/2026-i-aplicacion-riqueza-felidos/blob/main/index.qmd)
...
### Cajas de valor reactivas

Sobre las salidas principales se coloca una fila con tres **cajas de valor**. A diferencia de las cajas estáticas del @sec-tableros-control —cuyo valor se fija una sola vez al renderizar—, estas se **recalculan con cada cambio de filtro**. Por eso son una salida más del servidor: en la interfaz solo se reserva su espacio con un control de salida genérico, `uiOutput()`, y el servidor construye la caja completa con `bslib::value_box()` dentro de `renderUI()`.

Las tres cajas se agrupan en una fila (`## Row`) dentro de la página de la aplicación. Cada bloque reserva el espacio de una caja con `uiOutput()`:
...
Cada `outputId` (`"vb_registros"`, `"vb_especies"`, `"vb_areas"`) identifica la caja que el servidor generará con `renderUI()`.

### Controles de salida

Debajo de las cajas de valor, las tres salidas principales se organizan en una segunda fila con pestañas (`## Row {.tabset}`): una para el mapa, una para el gráfico y una para la tabla. Cada una se declara con la función `*Output()` correspondiente, que reserva el espacio en la interfaz. El contenido lo generará luego el servidor:
...
El bloque del servidor (`#| context: server`) contiene toda la lógica reactiva. Comienza con la función reactiva `felidos_filtrados()`, que aplica los dos filtros una sola vez, y continúa con las funciones `render*` que generan las salidas a partir de ella: las tres cajas de valor, el mapa, el gráfico y la tabla.
...
**Cajas de valor** (en el mismo bloque del servidor). Cada caja se construye con `value_box()` —indicando su título, valor, ícono (`bs_icon()`) y color (`theme`)— a partir de los registros filtrados. Se usan los mismos íconos y colores del tablero del @sec-tableros-control:
...
Nótese cómo cada función `render*` invoca `felidos_filtrados()` (con paréntesis). Gracias a la reactividad, cuando la persona usuaria cambia un filtro, todas las salidas —las tres cajas de valor, el mapa, el gráfico y la tabla— se recalculan automáticamente.

La @fig-shiny-flujo-reactivo resume este flujo para las tres salidas principales: los dos controles de entrada (`input$especie` e `input$area`) alimentan la función reactiva `felidos_filtrados()`, cuyo resultado comparten las tres funciones `render*` que generan el mapa, el gráfico y la tabla. Las cajas de valor siguen exactamente el mismo patrón: cada una invoca `felidos_filtrados()` dentro de su `renderUI()`.

![Flujo reactivo de la aplicación de félidos: los controles de entrada alimentan la función reactiva, cuyo resultado comparten las tres salidas.](img/shiny-flujo-reactivo.svg){#fig-shiny-flujo-reactivo .lightbox fig-alt="Diagrama del flujo reactivo: input$especie e input$area alimentan felidos_filtrados(), que a su vez alimenta output$mapa, output$grafico y output$tabla" fig-align="center"}
