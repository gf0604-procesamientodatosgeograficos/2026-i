...
En este capítulo se presentan las operaciones espaciales —las que involucran las geometrías de los objetos geoespaciales— tanto para datos vectoriales (con el paquete sf) como para datos raster (con el paquete terra). Se incluyen ejemplos basados en datos de cantones de Costa Rica, registros de presencia de félidos del [Sistema Mundial de Información en Biodiversidad (GBIF)](https://www.gbif.org/), temperatura media anual de [WorldClim](https://www.worldclim.org/) e imágenes del satélite [Sentinel-2](https://es.wikipedia.org/wiki/Sentinel-2).
...
La mayoría de los paquetes utilizados en este capítulo se instalaron en capítulos anteriores: `tidyverse` en el @sec-tidyverse, `DT` en el @sec-ggplot2-plotly, y `sf`, `terra` y `tmap` en el @sec-introduccion-geoespaciales. El único paquete nuevo es `leaflet`, que se utiliza al final del capítulo para visualizar el resultado del cálculo del NDVI sobre una imagen Sentinel-2 en un mapa interactivo.

```{r}
#| label: instalacion-paquetes
#| eval: false

# Instalación de leaflet (mapas interactivos)
...
# Carga de leaflet (mapas interactivos)
library(leaflet)
```

## Conjuntos de datos de ejemplo

A lo largo del capítulo se utilizan cuatro conjuntos de datos sobre Costa Rica: los **cantones** (polígonos), los **registros de presencia de félidos silvestres** (puntos), la **temperatura media anual** (raster) y dos **imágenes Sentinel-2 del refugio de vida silvestre Gandoca-Manzanillo** (raster multibanda). Las cargas detalladas se realizan más adelante, en la sección correspondiente a cada modelo de datos (sf para los dos primeros, terra para los demás). Sus fuentes son:
...
- **Imágenes Sentinel-2 de Gandoca-Manzanillo**: dos imágenes multibanda (2024-01-05 y 2024-06-18) del [refugio de vida silvestre Gandoca-Manzanillo](https://www.sinac.go.cr/ES/ac/aclac/rnvsgm/Paginas/default.aspx), captadas por la constelación [Sentinel-2](https://es.wikipedia.org/wiki/Sentinel-2) de la Agencia Espacial Europea.
...
2. Genere una capa con el cálculo del NDVI para la [imagen de Gandoca-Manzanillo tomada el 2024-06-18](https://github.com/gf0604-procesamientodatosgeograficos/2026-i/raw/refs/heads/main/datos/sentinel/gandoca-20240618.tif). Inclúyala en un mapa interactivo que permita comparar el cambio en la vegetación con respecto a la imagen del 2024-01-05.
